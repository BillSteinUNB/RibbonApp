import type { ApiResponse, ApiError, ApiRequestConfig, PaginatedResponse } from '../types/api';
...
class APIClient {
   ...
   */
  private getAbortController(abortController?: AbortController): AbortController {
    if (!abortController) {
  ...
  private async request<T>(
    ...
    try {
      ...
      // API errors that might be retried
      if (this.shouldRetry(error, attempt)) {
        await this.delay(this.getRetryDelay(attempt));
        return this.request<T>(endpoint, config, attempt + 1);
      }
